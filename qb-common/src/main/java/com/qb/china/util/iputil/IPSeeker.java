package com.qb.china.util.iputil;
...
public class IPSeeker {
	...
	public synchronized static IPSeeker getInstance(String ipDBFilePath){
		if(INSTANCE == null){
			INSTANCE = new IPSeeker(ipDBFilePath);
		}
		return INSTANCE;
	}

	private IPSeeker(String ipDBFilePath) {
		ipCache = new HashMap<String, IPLocation>();
		buf = new byte[100];
		b4 = new byte[4];
		b3 = new byte[3];
		try {
			ipFile = new RandomAccessFile(ipDBFilePath, "r");
		} catch (Exception e) {
			log.error("IP地址信息文件没有找到，IP显示功能将无法使用", e);
			ipFile = null;
		}
		// 如果打开文件成功，读取文件头信息
		if (ipFile != null) {
	 ...
	 */
	public synchronized List<IPEntry> getIPEntriesDebug(String s) {
	 ...
	 * @return 包含IPEntry类型的List
	 */
	public synchronized List<IPEntry> getIPEntries(String s) {
		List<IPEntry> ret = new ArrayList<IPEntry>();
		try {
			// 映射IP信息文件到内存中
			if (mbb == null) {
	...
	private IPLocation getIPLocation(int offset) {
		...
		if (b == REDIRECT_MODE_1) {
			...
			if (b == REDIRECT_MODE_2) {
				...
				location.setCountry(readString(countryOffset));
			// 读取地区标志
			location.setArea(readArea(mbb.position()));
		} else if (b == REDIRECT_MODE_2) {
			location.setCountry(readString(readInt3()));
			location.setArea(readArea(offset + 8));
		} else {
			location.setCountry(readString(mbb.position() - 1));
			location.setArea(readArea(mbb.position()));
		}
		return location;
	}
